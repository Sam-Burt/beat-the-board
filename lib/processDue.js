...
import { pickPoolTask } from "./missionPool";
...
export async function processDueMissions() {
  ...
  const { data: due } = await supabaseAdmin
    ...
    .select("id, player_id, title, text, random, points, reward_kind, trip_id, trips (status)")
    .is("sent_at", null)
  ...
  // Same generic filler used when the pool's genuinely empty AND when this
  // particular player's already had everything currently in it this event
  // — a queued surprise should still fire on schedule either way, not
  // silently vanish because the pool ran dry. template_id stays null for
  // it, so it's never tracked as "used" and can't block anything.
  const FALLBACK_TASK = {
    id: null,
    title: null,
    text: "Do something sneaky before the day's out 👀",
    points: 5,
    reward_kind: "points",
  };
  ...
  for (const row of due) {
    ...
    let rewardKind = row.reward_kind || "points";
    let templateId = null;
    if (row.random) {
      const result = await pickPoolTask(row.trip_id, row.player_id);
      const picked = result.status === "ok" ? result.task : FALLBACK_TASK;
    ...
    const { error: insertError } = await supabaseAdmin.from("missions").insert({
