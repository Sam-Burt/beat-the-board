...
export async function finalizeTrip(trip, forcedWinnerId = null) {
  const [rosterRes, eventsRes, adjustmentsRes, hotPotatoRes] = await Promise.all([
    supabaseAdmin
      .from("trip_players")
  ...
  const queryError = rosterRes.error || eventsRes.error || adjustmentsRes.error || hotPotatoRes.error;
  if (queryError) {
  ...
  const hotPotato = hotPotatoRes.data;
