...
export async function finalizeTrip(trip, forcedWinnerId = null) {
  ...
  // app open right when this fires (see the README's "no server cron"
  // note: this runs whenever any player's device happens to trigger the
  // deadline check, or the admin manually ends/declares it).
  await Promise.all(players.map((p) => recordNotification(p.id, {
    kind: "event_ended",
    title: "🏁 That's your lot",
    body: `${trip.name} is over. One of you won. The rest of you were content.`,
    url: "/",
  })));
  if (pushConfigured) {
