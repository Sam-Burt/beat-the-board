...
export async function finalizeTrip(trip, forcedWinnerId = null) {
  ...
  // Whoever's holding the Hot Potato at the deadline loses 10 points —
  // capped so that if they were sitting in 1st, it only drops them to 2nd
  // rather than further. Computed off standings BEFORE this penalty, then
  // folded in as a real point_adjustments row (same as any other award/
  // deduction) so it shows up in history and is included from here on.
  ...
  if (holderId && players.some((p) => p.id === holderId)) {
    ...
    if (holder && top && holder.id === top.id) {
      ...
      const cap = top.points - (secondPlace?.points ?? 0);
      deduction = Math.max(0, Math.min(10, cap));
    }
