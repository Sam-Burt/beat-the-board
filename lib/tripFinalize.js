...
export async function finalizeTrip(trip, forcedWinnerId = null) {
  ...
  // Same reasoning as the scheduled-missions cleanup above: a trade nobody
  // got round to accepting or declining before the whistle has nothing left
  // to resolve into. respond-trade already guards against a stray accept
  // slipping through after finalize, but cancelling here means the
  // recipient's blocking popup (components/TradeAlert.js) just isn't there
  // next time they open the app, rather than dead-ending on an error.
  await supabaseAdmin
    .from("mission_trades")
