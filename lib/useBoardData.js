...
export function useBoardData() {
  ...
  const loadAll = useCallback(async () => {
    ...
    if (tripId) {
      const [rosterRes, eventsRes, adjustmentsRes, leroyRes, boostsRes] = await Promise.all([
        ...
        supabase
          .from("leroy_sends")
          .select("id, sender_id, target_id, amount, sent_at, expires_at, resolved_at")
          .eq("trip_id", tripId)
  ...
  const saveEvent = useCallback(
    async ({ name, date, note, ranking }) => {
      ...
      // Leroy: same shape as the cheat flag — a pending steal resolves
      // the next time the target plays any round — but with a shelf
      // life. If that round happens outside the 18-hour window (see
      // app/api/leroy/send), it's already expired and just gets ignored;
      // nothing to clean up since the icon only ever shows for a
      // non-expired, unresolved row in the first place.
      const now = new Date();
      const pendingLeroys = leroySends.filter(
        (l) => !l.resolved_at && new Date(l.expires_at) > now && participants.includes(l.target_id)
      );
      for (const leroy of pendingLeroys) {
        ...
        await guardWrite(() =>
          supabase.from("leroy_sends").update({ resolved_at: now.toISOString() }).eq("id", leroy.id)
        );
      }
    ...
    [guardWrite, currentTrip, players, leroySends, pointBoosts]
  );
  ...
  // Who's currently got Leroy pending against them — unresolved and still
  // within the 18-hour window — drives the (unlabelled) icon on the board.
  // Recomputed on every render rather than memoized on a timer: it only
  // actually needs to be fresh when something else re-renders the page
  // anyway (a realtime update, a new trip), and an icon lingering for a
  // few extra seconds past expiry is harmless.
  const leroyTargetedIds = useMemo(() => {
    const now = new Date();
    return new Set(
      leroySends.filter((l) => !l.resolved_at && new Date(l.expires_at) > now).map((l) => l.target_id)
    );
  }, [leroySends]);

  // Who's currently sitting on an active Jackpot — same shape as
  // leroyTargetedIds above, drives the board badge.
