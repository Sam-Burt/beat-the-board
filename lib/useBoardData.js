...
export function useBoardData() {
  ...
  const loadAll = useCallback(async () => {
    ...
    if (tripId) {
      const [rosterRes, eventsRes, adjustmentsRes, leroyRes, boostsRes] = await Promise.all([
        ...
        supabase
          .from("point_boosts")
          .select("id, player_id, activated_at, expires_at, resolved_at")
          .eq("trip_id", tripId)
  ...
  const saveEvent = useCallback(
    async ({ name, date, note, ranking, category }) => {
      ...
      // Jackpot: same delayed-trigger, 18-hour shelf life as Leroy, but
      // self-directed and purely additive — doubling a share just means
      // adding another copy of it as a second adjustment, no need to
      // touch eventPoints() or the stored ranking at all. Deliberately
      // never interacts with Leroy either way: Leroy always steals a
      // flat 5 regardless of any boost in play, and a boost never
      // inflates what Leroy steals — otherwise everyone would just wait
      // for someone else to pop theirs before ever sending Leroy, and
      // the two tricks would stop being independent choices. A
      // cheat-flagged round is the one exception: that penalty already
      // voids the round to nothing, so there's no positive share left
      // for a boost to double.
  ...
  // Who's currently sitting on an active Jackpot — drives the board badge.
  // No Leroy equivalent any more: he's silent by design, so there's
  // nothing to show on the board until he's already struck and told the
  // target directly.
  const boostedIds = useMemo(() => {
    const now = new Date();
    return new Set(
      pointBoosts.filter((b) => !b.resolved_at && new Date(b.expires_at) > now).map((b) => b.player_id)
    );
  }, [pointBoosts]);
