...
export function useBoardData() {
  ...
  const saveEvent = useCallback(
    async ({ name, date, note, ranking }) => {
      ...
      // The cheat flag sticks around until whoever's wearing it wins
      // something — first place, solo or as part of a winning team — at
      // which point that win scores nothing (a compensating adjustment
      // cancels their share out to 0) AND costs them 5 more on top, then
      // the flag lifts.
      const CHEAT_PENALTY = 5;
      const winners = ranking[0] || [];
      const flagged = winners.filter((id) => players.find((p) => p.id === id)?.cheat_flagged);
      if (flagged.length > 0) {
        const shares = eventPoints({ ranking });
        for (const playerId of flagged) {
          ...
          await guardWrite(() =>
            supabase.from("point_adjustments").insert({
              ...
              note: "Cheating penalty — this win scores nothing, and costs 5 more",
            })
          );
          await guardWrite(() =>
    ...
    [guardWrite, currentTrip, players]
  );
